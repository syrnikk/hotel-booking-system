...
public class BookingSQL {
    ...
    public static final String SELECT_RESERVATIONS_SQL =
            """
            SELECT r.id, h.nazwa, tp.typ, SUM(tp.cena) AS cena, r.przyjazd_data, r.odjazd_data, COUNT(p.id) as ilosc_pokoji FROM rezerwacja r INNER JOIN pokoj_rezerwacja pr
            ...
            WHERE r.uzytkownik_id = ? GROUP BY r.id, r.przyjazd_data, r.odjazd_data, h.nazwa, tp.typ;
            """;
}
